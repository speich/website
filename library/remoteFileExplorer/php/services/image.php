...
$pathFsRoot = $rfeConfig['paths']['fileSystemRoot'];
$pathFsCache = $rfeConfig['paths']['thumbnailCache'];

if ($resource && file_exists($pathFsRoot.ltrim($resource, '/'))) {
    ...
    if (!file_exists($pathFsCache.$id.".jpg")) {
        $filters = ['unsharpMask' => []];
        $imgWidth = property_exists($data, 'w') ? $data->w : 256;
        $jpgQuality = 80;
        $imgSrc = $pathFsRoot.ltrim($resource, '/');
        $imgTrg = $pathFsCache.$id.".jpg";
