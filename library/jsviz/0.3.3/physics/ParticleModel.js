	...
	draw: function( force ) {
		...
		for ( var i=0, l=particles.length; i<l; i++ ) {
			...
			if( this.boundsLeft ) { //only check if the bounds have been set
				if ( particle.positionX < this.boundsLeft+(particle.width/PosCheck)/skewX ) {
					particle.positionX = this.boundsLeft+(particle.width/PosCheck)/skewX;
				} else if ( particle.positionX > (this.boundsRight-(particle.width/PosCheck)/skewX) ) {
					particle.positionX = this.boundsRight-(particle.width/PosCheck)/skewX;
				}
	
				if ( particle.positionY < this.boundsTop+(particle.height/PosCheck/skewY) ) {
					particle.positionY = this.boundsTop+(particle.height/PosCheck/skewY);
				} else if ( particle.positionY > (this.boundsBottom-(particle.height/PosCheck/skewY)) ) {
					particle.positionY = this.boundsBottom-(particle.height/PosCheck/skewY);
				}
			}

			var newDrawPositionX = Math.round(particle.positionX*PosCheck)/PosCheck;
			var newDrawPositionY = Math.round(particle.positionY*PosCheck)/PosCheck;
...
// todo: I'm only pulling one node from the active collection per iteration
		...
		if ( magLen > 0 && this.activeMagnets[0] > 50 ) {
			this.activeMagnets.shift();
		}
